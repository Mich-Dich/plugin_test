...
namespace GLT::io {
	...
	std::string read_file(const std::filesystem::path &filepath) {
		...
		VALIDATE(stream.is_open(), return std::string(), "", "File [[]] could not be opend", filepath);
	...
	bool write_file(const std::filesystem::path &file_path, const std::vector<char> &content_buffer) {
		...
		LOG(trace, "Wrote content to file at [{}] with length [{}]", file_path.generic_string(), content_buffer.size());
		return true;
	}
	...
	bool copy_file(const std::filesystem::path &full_path_to_file, const std::filesystem::path &target_directory) {
		...
		try {
			...
			VALIDATE(std::filesystem::exists(full_path_to_file), return false, "", "Source file does not exist [{}]", full_path_to_file.string());
			...
			if (!std::filesystem::exists(target_directory))	{ // Check if the target directory exists, if not, create it
				...
				try {
					...
					LOG(error, "Filesystem error: code [{}] message [{}]", e.code().value(), e.what())
					return false;
				}
			}
			...
			try {
				...
				LOG(error, "Filesystem error: code [{}] message [{}] source path: [{}] Target path: [{}]", e.code().value(), e.what(), e.path1().string(), e.path2().string())
				return false;
			}
			return true;
